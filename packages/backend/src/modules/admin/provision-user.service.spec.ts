...
describe("ProvisionUserService owner protection", () => {
  it("prevents a manager from creating another manager", async () => {
    const prisma = { appUser: { findUnique: jest.fn() }, factory: { findUniqueOrThrow: jest.fn() } };
    const service = new ProvisionUserService(prisma as any);

    await expect(service.provision("factory-a", "manager", "Manager Two", "manager2@example.com", "temporary-pass", "manager"))
